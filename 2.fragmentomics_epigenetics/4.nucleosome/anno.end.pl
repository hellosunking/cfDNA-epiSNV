...
foreach my $file ( @ARGV ) {
	open IN, "$file" or die( "$!" );
	while( <IN> ) {
		my @l = split /\t/;	##chr1	10016	10109	chr1	10005	10206	DANPOS	93
		next if $l[3] eq '.';

		my $nc = $l[5] - 100;	## nucleosome center
		my ($U, $D) = ($IMPOSSIBLE, $IMPOSSIBLE);

		if( $l[1] >= $l[4] ) {	## left hand is within the nucleosome
			$U = $l[1] + 1 - $nc;
		}
		if( $l[2] <= $l[5] ) {	## right hand is within the nucleosome
			$D = $l[2] - $nc;
		}
