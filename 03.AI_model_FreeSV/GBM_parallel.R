...
for (rep in 1:100) { 
  ...
  foreach(fold = 1:10, .packages = c("caret", "pROC"), .verbose = TRUE) %dopar% {
    ...
    name=paste0("GBM.model_rep_",rep,"_fold_",fold,".rds")
    saveRDS(model, name)
    ...
    write.table(train_pred_t, file = paste0("train_pred_rep_", rep, "_fold_", fold, ".txt"), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
    write.table(test_pred_t, file = paste0("test_pred_rep_", rep, "_fold_", fold, ".txt"), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

    # output best param
    best_params_file <- paste0("best_params_rep_", rep, "_fold_", fold, ".txt")
