...
plot( roc.1, lwd=2, col="darkorange", cex.lab=1.2, main=plot_title);
plot( roc.2, lwd=2, col="blue", add=T );
...
for (i in 1:7) {
  ...
  coords <- coords(roc_obj, "all", ret = c("threshold", "sensitivity", "specificity"))
  
  target_coord <- coords[coords[, "specificity"] >= 0.95, ][which.max(coords[coords[, "specificity"] >= 0.95, "sensitivity"]), ]
  
}

legend( 'bottomright', legend=paste0(c("BRCA", "COREAD", "ESCA", "STAD", "LIHC", "NSCLC", "PACA"), ", AUC=", c(auc.1, auc.2, auc.3, auc.4, auc.5, auc.6, auc.7), ", P=", c(p.1, p.2, p.3, p.4, p.5, p.6, p.7)),
...
for (i in 1:7) {
  ...
  # Calculate the number of positive cases detected at the determined threshold
